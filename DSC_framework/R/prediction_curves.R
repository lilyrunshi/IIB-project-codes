...
read_model3_simulation <- function(row, root_dir, frequency = 1.0) {
  ...
  if (is.null(time_points) && !is.null(row$simulate.x)) {
    ...
    if (is.null(time_points)) {
      stop(
        paste0(
          ...
          "exports either the latent time vector or the design matrix."
        ),
        call. = FALSE
      )
    }
    if (frequency != 1.0) {
      warning(
        paste0(
          "Recovered time points are periodic modulo 1/frequency. Provide simulate.latent ",
          "for exact timings when using frequency != 1."
        )
      )
    }
  }
  if (is.null(time_points)) {
    stop(
      paste0(
        "Latent time points were not found. Rerun dscquery() with targets including ",
        "simulate.latent or simulate.x so they can be reconstructed."
      ),
      call. = FALSE
    )
  }
...
prepare_prediction_curves <- function(dsc_table, root_dir, frequency = 1.0) {
  required_columns <- c("simulate.y", "simulate.w_true", "analyze.fit")
  ...
  if (!"simulate.latent" %in% names(dsc_table) && !"simulate.x" %in% names(dsc_table)) {
    stop(
      paste0(
        "Neither simulate.latent nor simulate.x is available in the DSC table. ",
        "Include at least one of these outputs so sampling times can be reconstructed."
      ),
      call. = FALSE
    )
  }

  latent_keys <- if ("simulate.latent" %in% names(dsc_table)) {
    coerce_id_string(dsc_table$simulate.latent)
  } else {
    coerce_id_string(dsc_table$simulate.x)
  }
  ...
  if ("simulate.latent" %in% names(dsc_table)) {
    sim_cols <- c(sim_cols, "simulate.latent")
  }
  if ("simulate.x" %in% names(dsc_table)) {
    sim_cols <- c(sim_cols, "simulate.x")
  }
